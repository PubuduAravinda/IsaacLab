...
# UPRIGHT_WEIGHT — UPDATED after real Go2 hardware test showed persistent
# elevated tilt (mean 19.1 deg, 41.6% of samples >20 deg) that sim never
# predicted for the same checkpoint (sim: 0% >20 deg). Strengthened 3x as
# a reasoned hypothesis, NOT a confirmed fix the way the action_rate
# change was -- there we had a direct 20-25x weight gap explaining a
# measured 3.5x behavior gap. Here we don't have equivalently direct
# evidence the reward is the cause vs. hardware-side factors (battery
# charge, real floor friction vs Test1/2/3/5 calibration, mechanical
# wear) -- rule those out in parallel, don't assume this alone fixes it.
UPRIGHT_WEIGHT    = -1.5    # was -0.5
...
class Go2EnvSparseRough(Go2Env):
    ...
    def __init__(self, cfg, render_mode=None, **kwargs):
        ...
        print(f"  UPRIGHT strengthened -0.5->-1.5, hip_posture loosened "
              f"-3.0->-1.0, hip_reg loosened -2.0->-0.8  "
              f"(hypothesis: real-hw tilt [19.1 deg mean, 41.6%>20deg, "
              f"sim showed 0%] connected to overly-restricted hip range "
              f"[14-50% of available]. UNCONFIRMED -- validate hip usage "
              f"AND tilt together next run, and rule out battery/floor/"
              f"calibration factors in parallel, don't assume this alone "
              f"is the fix.)")
        print("="*72 + "\n")
    ...
    def _get_rewards(self):
        ...
        # LOOSENED after observing hip usage stayed narrow (14-50% of
        # available +/-0.30 rad range) across multiple training runs,
        # alongside persistent real-hardware tilt sim never predicted.
        # Hypothesis: these penalties were structurally preventing the
        # policy from using hip motion for lateral balance correction --
        # loosened here to test that; not confirmed, watch hip usage AND
        # tilt together in the next validation run, not just one or the
        # other, to see whether they move together (supporting the
        # hypothesis) or independently (meaning something else is going
        # on with tilt specifically).
        hip_deviation = (self._robot.data.joint_pos[:, :4]
                         - self._robot.data.default_joint_pos[:, :4])
        r_hip_posture = -1.0 * torch.sum(hip_deviation**2, dim=1)   # was -3.0
        r_hip_reg     = -0.8 * torch.sum(self._actions[:, :4]**2, dim=1)  # was -2.0
