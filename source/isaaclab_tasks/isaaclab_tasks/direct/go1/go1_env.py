...
class Go1Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Go1FlatEnvCfg | Go1RoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._trunk_idx = self._robot.find_bodies("trunk")[0]  # Matches body name: trunk
    ...
    def _setup_scene(self):
        ...
        # Debug: Print stage hierarchy and list foot paths
        ...
        for prim in stage.Traverse():
            ...
            if path.pathString.endswith("_foot"):
                foot_paths.append(path.pathString)
        print(f"✅ Found foot paths: {foot_paths}")
    ...
    def _apply_carpet_material(self):
        ...
        print(f"🔍 Ground transform: {ground_xform.GetLocalTransformation()}")
        ...
        try:
            ...
            if os.path.exists(texture_path):
                ...
                transform_2d.CreateInput("scale", Sdf.ValueTypeNames.Float2).Set(Gf.Vec2f(10.0, 10.0))  # Reduced scale
                ...
                print("⚠️ Carpet texture not found, using purple fallback")
            ...
            bound_material = binding_api.GetDirectBinding().GetMaterialPath()
            print(f"🔍 Ground material bound to: {bound_material}")
        except Exception as e:
    ...
    def _get_foot_contact_binary(self) -> torch.Tensor:
        try:
            if not hasattr(self._contact_sensor.data, 'net_forces_w'):
                print("⚠️ Contact sensor data unavailable, returning zeros")
            ...
            print(f"❌ Contact sensor error: {e}")
            return torch.zeros(self.num_envs, 4, device=self.device)
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        root_state[:, 2] = 0.5  # Increased initial height
        root_state[:, 7:10] = 0.0  # Zero linear velocity
        root_state[:, 9] = 0.1     # Small upward velocity
